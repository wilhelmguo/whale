...
public class ShiroRealm extends AuthorizingRealm {
  ...
  protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
    ...
    String[] userAndCompany = token.getUsername().split(",jy,");
    if (null == userAndCompany || userAndCompany.length != 2) {
      throw new UnknownAccountException(); /*找不到帐号异常*/
    }
    String username = new String(userAndCompany[0]);//用户名
    String password = new String(token.getPassword());//密码
    Account a;
    if (!Const.SUPERUSER.equals(username)){
      a = accountService.findFormatByLoginName(username,userAndCompany[1]);//通过登录名 寻找用户
    }else {
      a = accountService.findFormatByLoginName(username);//通过登录名 寻找用户
    }
    if (a != null) {
