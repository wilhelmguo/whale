...
public class LoginController extends BaseController<Object> {
    ...
    if (null != KEYDATA && KEYDATA.length == 4) {
      ...
      if (null == code || "".equals(code)) {
        ...
        if (StringUtils.isNotEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)) {
          // shiro加入身份验证
          UsernamePasswordToken token = new UsernamePasswordToken(username + ",jy," + companyID, password.toUpperCase());
        ...
        if (StringUtils.isEmpty(errInfo)) {
          ...
          session.setAttribute(Const.SESSION_COMPANY, companyID);
        }
      }
    } else {
      errInfo = "error";  //缺少参数
  ...
  public String logout(HttpServletRequest request, HttpSession session) {
    ...
    session.removeAttribute(Const.SESSION_COMPANY);
