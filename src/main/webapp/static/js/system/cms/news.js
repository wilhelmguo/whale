$(function () {
	...
	$('#addBtn').on('click', function(e) {
		...
		JY.Model.wideedit("800","auDiv","新增",function(){
			 if(JY.Validate.form("auForm")){
				 ...
				 JY.Ajax.doRequest("auForm",jypath +'/backstage/cms/add',null,function(data){
				     that.dialog("close");
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			JY.Model.confirm("确认要删除选中的数据吗?",function(){
				JY.Ajax.doRequest(null,jypath +'/backstage/cms/delBatch',{chks:chks.toString()},function(data){
					JY.Model.info(data.resMsg,function(){search();});
				});
			});
		}
...
function del(id){
	JY.Model.confirm("确认删除吗？",function(){
		JY.Ajax.doRequest(null,jypath +'/backstage/cms/del',{id:id},function(data){
			JY.Model.info(data.resMsg,function(){search();});
		});
	});
}
	...
	JY.Ajax.doRequest(null,jypath +'/backstage/cms/find',{id:id},function(data){
		...
		JY.Model.wideedit("800","auDiv","修改",function(){
			...
			if(JY.Validate.form("auForm")){
				...
				JY.Ajax.doRequest("auForm",jypath +'/backstage/cms/update',null,function(data){
					if(data.res==1){
						that.dialog("close");
						JY.Model.info(data.resMsg,function(){search();});
					}else{
