...
function getMsg() {
    JY.Ajax.doRequest(null, jypath + '/backstage/msg/findByPage', {"status": 0}, function (data) {
            if (data.res == 1) {
                var result = data.obj;
                var list = result.list;
                var results = list.results;
                var totalRecord = list.totalRecord;
                $("#totalNum").text(totalRecord);
                var html = "";
                html += '<li class="dropdown-header">';
                html += '<i class="icon-envelope-alt"></i>';
                html += ' 共' + totalRecord + '条消息';
                html += ' </li>';
                if (results != null && results.length > 0) {
                    var num = results.length >= 4 ? 4 : results.length;
                    for (var i = 0; i < num; i++) {
                        html += '<li>';
                        html += '<a href="#">'
                        html += '<span class="msg-body" style="max-width: 250px">';
                        html += ' <span  class="msg-title">';
                        html += '  加班流程-李四-2016-08-21 21:23';
                        html += '<span  class="blue">(已同意)</span>';
                        html += '   </span>';
                        html += '   <span class="msg-time">';
                        html += '   <i class="icon-time"></i>';
                        html += '   <span>1分钟以前</span>';
                        html += '   </span>';
                        html += '  </span>';
                        html += '   </a>';
                        html += '  </li>';
...
function showMsg() {
    parent.TabControlAppend("menu3aa415da626042e4b6a9bd8b0bdee4ad", "我的消息",
        "/whale/backstage/msg/index?menu=menu3aa415da626042e4b6a9bd8b0bdee4ad");
}
                        ...
                        click: function () {
                            ...
                            if ($("#avatarSetting").hasClass("active")) {
                                ...
                                if (JY.Validate.form("pwSettingFrom")) {
                                    ...
                                    if (opwd != npwd) {
                                        if (npwd == qpwd) {
                                            JY.Ajax.doRequest(null, jypath + '/backstage/account/preResetPWD', {
                                                opwd: opwd,
                                                npwd: npwd,
                                                qpwd: qpwd
                                            }, function (data) {
                                            ...
                                            $("#pwSettingFrom input[name='npwd']").tips({
                                                side: 1,
                                                msg: "您输入的密码与确认密码不一致！",
                                                bg: '#FF2D2D',
                                                time: 1
...
function logout() {
    JY.Model.confirm("确认要退出吗？", function () {
