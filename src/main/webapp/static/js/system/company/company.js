$(function () {
    ...
    $('#editOrgBtn').on('click', function (e) {
        ...
        JY.Ajax.doRequest(null, jypath + '/backstage/org/role/findOrg', {id: orgId}, function (data) {
            if (data.res == 1) {
                setOrgForm(data);
                JY.Model.edit("auOrgDiv", "修改公司", function () {
                    if (JY.Validate.form("auOrgForm")) {
                        var that = $(this);
                        JY.Ajax.doRequest("auOrgForm", jypath + '/backstage/org/role/updateCompany', null, function (data) {
                            that.dialog("close");
                            JY.Model.info(data.resMsg, function () {
                                refreshOrgTree();
                            });
    ...
    $('#delOrgBtn').on('click', function (e) {
        ...
        JY.Model.confirm("确认要删除【" + ptreeNode.name + "】吗？", function () {
            JY.Ajax.doRequest(null, jypath + '/backstage/org/role/delCompany', {id: orgId}, function (data) {
                JY.Model.info(data.resMsg, function () {
                    loadOrgTree();
                });
            });
        ...
        if (JY.Object.notNull(orgId)) {
            ...
            JY.Model.edit("auDiv", "新增角色", function () {
                if (JY.Validate.form("auForm")) {
                    var that = $(this);
                    JY.Ajax.doRequest("auForm", jypath + '/backstage/org/role/companyAdd', {orgId: orgId}, function (data) {
                        that.dialog("close");
                        JY.Model.info(data.resMsg, function () {
                            search();
                        });
...
function loadOrgTree() {
    JY.Ajax.doRequest(null, jypath + '/backstage/org/role/getSuperUserOrgTree', null, function (data) {
        //设置
...
function getbaseList(init) {
    ...
    JY.Model.loading();
    JY.Ajax.doRequest("baseForm", jypath + '/backstage/org/role/findOneRole', null, function (data) {
...
function addAdmin(id) {
    ...
    JY.Model.confirm("确认生成管理员账户吗？默认用户名:admin密码:1", function () {
        JY.Ajax.doRequest(null, jypath + '/backstage/company/addAdmin', {id: id}, function (data) {
            JY.Model.info(data.resMsg, function () {
                search();
            });
        });
